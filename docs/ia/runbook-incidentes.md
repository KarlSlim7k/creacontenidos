...
2. `content-engine`: si la llamada al modelo falla para un tema, dejar el `topic` en `status='new'` para reintentar en el siguiente ciclo (ver spec de la capa). Antes de eso hay **un** reintento automático contra `AI_MODEL_FALLBACK` (`apps/api/src/lib/ai-client.js`): mismo Nous Portal, solo cambia el `model` — cubre modelo retirado, rate limit y errores puntuales del primario. Con la variable vacía no hay fallback y el error se propaga como antes. Si el log dice `Nous Portal falló con modelo primario y de respaldo`, cayeron los dos: es problema del Portal, no del modelo, y aplica el punto 3. Lo que sigue sin existir es fallback de **proveedor** (cadena nativa tipo Hermes/Portal completo, ver [`politica-ia-y-gate-editorial.md`](./politica-ia-y-gate-editorial.md) §1.2).
